...
##    Hands are private: each peer receives only its own cards.
...
func start_server(port: int = DEFAULT_PORT) -> Error:
	...
	add_child(game)
	game.awaiting_action.connect(_sync_state.unbind(2))
	game.trick_won.connect(_sync_state.unbind(2))
	game.round_ended.connect(_sync_state.unbind(2))
	game.game_ended.connect(_sync_state.unbind(1))
	_sync_state()
...
func s_action(seat: int, action: String, args: Array) -> void:
	...
	match action:
		...
		"play_card":
			if args.size() < 2:
				return
			game.play_card(player, _hand_card(player, args[0], args[1]))
		"concede":
			game.concede(player)
		"pass_redeal":
...
func _snapshot(for_seat: int) -> Dictionary:
	...
	return {
		...
		"hand": hand,  # own cards only, as [[suit, rank], ...]
